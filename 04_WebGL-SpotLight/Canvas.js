...
var lightAmbient = [ 0.9, 0.1, 0.1];
var lightDiffuse = [ 1.0, 1.0, 1.0]; // white diffuse light
var lightSpecular = [ 5.0, 7.0, 1.0];
var lightPosition = [-6.0, 1.5, 2.0];
var lightAngle = 0.0;

var directionPass = [1.0, 0.0, 2.0];
...
function initialize()
{
    ...
    "float theta = dot(normalizedTranformedNormals, normalize(-direction)); " +
    "if(uKeyPress == 1)" +
    "{" +
    "if(theta > cutOff)" +
    "{" +
    ...
    direction = gl.getUniformLocation(shaderProgramObject, "direction");
	cutoff = gl.getUniformLocation(shaderProgramObject, "cutoff");
...
{ 
    ...
    //mat4.rotate(modelMatrix, modelMatrix, 45.0*Math.PI/180.0, [0.0, 1.0, 0.0]);
    ...
    if (bLightingEnabled == true) {
        ...
        gl.uniform3fv(direction, center);
        gl.uniform1f(cutoff, Math.cos(10)); //Math.cos(0.2181662));
        gl.uniform1f(constant, 0.015);
		gl.uniform1f(linear, 0.09);
		gl.uniform1f(quadric, 0.000092);
    }
    else {
